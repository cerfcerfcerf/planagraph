...
export function useTemplate(id: number): Promise<Task> {
  return request(`/templates/${id}/use`, { method: "POST" });
}

export function fetchLazySuggestions(title: string, notes?: string | null) {
  return request<{ suggestions: string[] }>("/lazy_suggestions", {
    method: "POST",
