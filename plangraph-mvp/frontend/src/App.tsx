...
import { api } from "./api";
import type { NowResponse, PlannedItem, Reminder, ScheduleItem } from "./types";
import "./App.css";
...
export default function App() {
  ...
  const [tasks, setTasks] = useState<PlannedItem[]>([]);
  const [taskMenu, setTaskMenu] = useState<TaskMenuState | null>(null);
  const [rescheduleModal, setRescheduleModal] = useState<PlannedItem | null>(null);
  ...
  const handleReminderAction = async (
    reminder: Reminder,
    action: "done" | "snooze" | "delete" | "reschedule",
    time?: string
  ) => {
    await api.taskAction(reminder.id, {
      action,
      target: "reminder",
  ...
  const dueNow = nowData?.due_reminders ?? [];
  const dueNowVisible = showAllDue ? dueNow : dueNow.slice(0, 3);
  const nextSix = nowData?.next_items ?? [];
    ...
    <article className="reminder-card" key={reminder.id}>
      <div className="reminder-card-header">
        ...
        <div className="pill-row">
          <span className="why" title={reminder.reason ?? "No reason"}>Why</span>
      ...
      {showActions && (
        <div className="actions">
          <button onClick={() => handleReminderAction(reminder, "done")}>Done</button>
          <button className="ghost" onClick={() => handleReminderAction(reminder, "snooze")}>
            Snooze
          </button>
          <button className="ghost" onClick={() => handleReminderAction(reminder, "reschedule")}>
            Reschedule
          </button>
          <button className="ghost" onClick={() => handleReminderAction(reminder, "delete")}>
            Delete
          </button>
        </div>
      )}
  ...
  return (
    <div className="app">
      <header className="hero">
        <div>
          <h1>Plangraph — local-first planner</h1>
      ...
      {activeTab === "now" && (
        <section className="stack">
          <section className="card">
            ...
            {dueNow.length === 0 ? (
              <p className="muted">{nowData?.message ?? "You’re clear for now."}</p>
            ) : (
                ...
                {nextSix.map((item) => (
                  <div key={item.id} className="timeline-row">
                    <div>
                      <h3>{item.title}</h3>
                      <p className="muted">
                        {item.planned_start ?? "No time"} {item.planned_end ? `– ${item.planned_end}` : ""}
                      </p>
                    </div>
                    <span className="why" title={item.reason ?? "No reason"}>Why</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="actions">
              ...
              <button
                onClick={async () => {
                  ...
                  const entry = await api.createEntry(pasteText, today);
                  await api.plan(today, "07:00", "21:00", entry.items);
                  setActiveTab("now");
                  await refreshNow();
                }}
                ...
                tasks.map((task) => (
                  <div
                    ...
                    onContextMenu={(event) => {
                      ...
                      setTaskMenu({ item: task, anchor: { x: event.clientX, y: event.clientY } });
                    }}
                  >
                    <div>
                      <h3>{task.title}</h3>
                      <p className="muted">
                        {task.planned_start ? `${task.planned_start}${task.planned_end ? `–${task.planned_end}` : ""}` : "No time"}
                      </p>
                    </div>
                    <div className="task-meta">
                      <span className="pill">{task.status}</span>
                      <span className="why" title={task.reason ?? "No reason"}>Why</span>
                    </div>
                  </div>
                ))
              )}
        ...
        <div className="context-menu" style={{ top: taskMenu.anchor.y, left: taskMenu.anchor.x }}>
          <button
            onClick={async () => {
              ...
              await api.taskAction(taskMenu.item.id, { action: "done", target: "planned" });
              setTaskMenu(null);
              await loadTasks();
            }}
          >
          ...
          <button
            onClick={async () => {
              if (!taskMenu.item.id) return;
              await api.taskAction(taskMenu.item.id, { action: "snooze", target: "planned" });
              setTaskMenu(null);
              await loadTasks();
            }}
          >
          ...
          <button
            onClick={() => {
              setRescheduleModal(taskMenu.item);
              setRescheduleTime(taskMenu.item.planned_start ?? "");
              setTaskMenu(null);
            }}
          >
            Reschedule
          ...
          <button
            onClick={async () => {
              if (!taskMenu.item.id) return;
              await api.taskAction(taskMenu.item.id, { action: "delete", target: "planned" });
              setTaskMenu(null);
              await loadTasks();
            }}
          >
      ...
      {rescheduleModal && (
        <div className="modal-overlay">
          <div className="modal">
            <div className="card-header">
              <h2>Reschedule</h2>
              <button className="ghost" onClick={() => setRescheduleModal(null)}>
                Close
              </button>
            </div>
            <label className="field">
            ...
            <div className="actions">
              <button
                onClick={async () => {
                  ...
                  await api.taskAction(rescheduleModal.id, {
                    action: "reschedule",
                    target: "planned",
