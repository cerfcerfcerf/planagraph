# Plangraph — local-first planner (backend)
...
```bash
curl "http://localhost:8000/tasks?range=week&type=task&status=all"

curl -X POST http://localhost:8000/tasks/quick_add \
  -H "Content-Type: application/json" \
  -d '{"title":"Quick task","date":"2024-01-02","time":"10:00","priority":1}'

curl -X POST http://localhost:8000/tasks/123/action \
  -H "Content-Type: application/json" \
  -d '{"action":"done","target":"planned"}'
```
